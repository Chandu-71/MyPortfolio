    ...
    <section className='w-full min-h-screen bg-dot-white/[0.08] flex flex-col items-center justify-center py-16 px-2 md:px-4'>
      ...
      <div className='relative w-full flex flex-row items-center justify-center'>
        ...
        <div className='md:max-w-2/3 relative flex-1 flex flex-col items-center'>
          {/* Vertical Line */}
          <div className='absolute left-1/2 top-0 -translate-x-1/2 h-full w-0.5 bg-blue-600 shadow-[0_0_12px_2px_#7f5fff99] rounded-full z-0' />
            ...
            {educationData.map((edu, idx) => {
              ...
              return (
                <div key={edu.institution} className='relative flex items-center w-full min-h-[120px] mb-12 last:mb-0'>
                  {/* Timeline Dot */}
                  <div className='absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 max-md:hidden'>
                    <div className='w-4.5 h-4.5 rounded-full bg-black border-3 border-blue-600 shadow-[0_0_12px_#7f5fff99]' />
                  </div>

                  {/* Education Card Wrapper */}
                  <div
                    className={`w-full md:w-1/2 ${
                      isRight ? 'ml-auto pl-8 md:pl-12' : 'mr-auto pr-8 md:pr-12'
                    } flex flex-col items-${isRight ? 'start' : 'end'} relative`}
                  >
                    <div className='relative bg-black/30 border border-blue-500/40 shadow-md hover:shadow-blue-500/30 transition-all duration-300 backdrop-blur rounded-xl px-6 py-5 max-w-md w-full'>
                      {/* Connectors */}
                      {isRight && (
                        <div className='absolute top-1/2 -translate-y-1/2 -left-8 md:-left-12 w-8 md:w-12 h-[2px] bg-blue-500/60 hidden md:block z-0' />
                      )}
          ...
          {/* Bottom Endpoint Icon */}
          <div className='absolute left-1/2 bottom-0 -translate-x-1/2 translate-y-1/2 z-20 pointer-events-none'>
